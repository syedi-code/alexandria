...
describe('getLibraryBooks', () => {
	it('returns every work with enrichment counts and totals', async () => {
		const result = await getLibraryBooks(db.d1);
	...
	it('excludes superseded quotes, notes and essays from the counts', async () => {
		const { books } = await getLibraryBooks(db.d1);
	...
	it('filters on has_pdf in SQL and on decade in memory', async () => {
		expect(
			(await getLibraryBooks(db.d1, { hasPdf: true })).books
		).toHaveLength(2);
		expect(
			(await getLibraryBooks(db.d1, { hasPdf: false })).books
		).toHaveLength(3);
		expect(
			(await getLibraryBooks(db.d1, { decade: 'pre-1900' })).books
		).toHaveLength(2);
	});

	it('searches title and author case-insensitively', async () => {
		expect(
			(await getLibraryBooks(db.d1, { search: 'gravity' })).books
		).toHaveLength(1);
		expect(
			(await getLibraryBooks(db.d1, { search: 'NIETZSCHE' })).books
		).toHaveLength(2);
	});
	...
	it.each([
		...
		const { books } = await getLibraryBooks(db.d1, { sort });
		expect(books.map((b) => b.title)).toEqual(expected);
	});

	it('honours limit and offset', async () => {
		const { books } = await getLibraryBooks(db.d1, { limit: 2, offset: 1 });
...
describe('getBookDetail', () => {
	it('returns null for an unknown id', async () => {
		expect(await getBookDetail(db.d1, 'nope')).toBeNull();
	});

	it('returns the work with its author, writing and media', async () => {
		const detail = (await getBookDetail(db.d1, ids.bookBeyondGoodAndEvil))!;
	...
	it('leaves author null when the work has no author row', async () => {
		const detail = (await getBookDetail(db.d1, ids.bookAnonymousPamphlet))!;
	...
	it('matches the recorded payload shape', async () => {
		expect(
			await getBookDetail(db.d1, ids.bookBeyondGoodAndEvil)
		).toMatchSnapshot();
	});
});
