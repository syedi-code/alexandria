...
is the audience of the Cloudflare Access application in front of *that*
deployment, and the developer's `.env` held the staging one while production's
secret held production's. `npm run sync:secrets` pushes every `.env` variable
to **production**, so one run would have pointed production at staging and
made every sign-in fail with `JWT_VERIFICATION_FAILED`. `PER_ENVIRONMENT` in
`cli/setup/sync-secrets.ts` now refuses those, and `CLOUDFLARE_API_TOKEN` is
blacklisted: a worker secret is readable by every route in the worker, and
that token can rewrite the Access policies in front of it. `POLICY_AUD` takes
a comma-separated list, one audience per `/login/<idp>` app plus the gate's.
